...
import com.example.myapplication.common.errorTypes.ErrorValues
...
object ErrorHandler {
    ...
    fun parseRetrofitAPIError(retrofitError: String?): Throwable?{
        when (retrofitError) {
            "InvalidRequest" -> {
                return Throwable(ErrorValues.INVALIDREQUEST)
            }
            else -> {
                return null
            }
